...
def _minus_logLikle_airma(theta, df, order, n_sample, n_particle):
    # obtain error series recursively
    def error_recursion_airma():
        ar_order = order[0]
        ma_order = order[1]

        ar_params = theta[:, :ar_order]
        ma_params = theta[:, ar_order:-1]
        std_param = theta[:, -1]
        ...
        error_upper = std_param[:, np.newaxis] * np.repeat(
            expect_upper, n_particle, axis=0
        )
        error_lower = std_param[:, np.newaxis] * np.repeat(
            expect_lower, n_particle, axis=0
        )

        upper = df["Upper"]["data"]
        lower = df["Lower"]["data"]
        for i in range(len(upper) - ar_order):
            rec_upper = (
                upper[i + ar_order]
                - (ar_params * upper[i : i + ar_order]).sum(axis=1)
                + (ma_params * error_upper[:, -ma_order:]).sum(axis=1)
            )
            rec_lower = (
                lower[i + ar_order]
                - (ar_params * lower[i : i + ar_order]).sum(axis=1)
                + (ma_params * error_lower[:, -ma_order:]).sum(axis=1)
            )
    ...
    error_upper, error_lower = error_recursion_airma()
    # * For more accurate estimation, drop some errors(4%) which might be
    # * greatly effected by initial values
    n_obs = df["Upper"]["data"].shape[0]
    drop_num = min(5 * order[1], n_obs / 3)
...
class AIRMA(Base):
    """Auto-Interval-Regressive Moving Averaging Model (AIRMA) Model

    Attributes:
        endog (pandas.DataFrame): The observed interval time-series process.
        order (tuple): The order of AIRMA model:
            - The first element is the length of AR lags.
            - The second element is the length of MA lags.
    ...
    def __repr__(self):
        return f"Auto-Interval-Regressive Moving Averaging Model(order=({self.order[0]},{self.order[1]}), n={self._n_obs})"

    def fit(self, n_particle=100, sigma_upper=5, iters=500):
        ...
        """
        params_upper_bound = np.hstack((np.ones(sum(self.order)), sigma_upper))
        params_lower_bound = np.hstack((-1 * np.ones(sum(self.order)), 0))
        bounds = (params_lower_bound, params_upper_bound)
        options = {"c1": 0.5, "c2": 0.3, "w": 0.9}
        optimizer = GlobalBestPSO(
            n_particles=n_particle,
            dimensions=sum(self.order) + 1,
            options=options,
            bounds=bounds,
        )
        ...
        _, pos_back = optimizer.optimize(
            _minus_logLikle_airma, iters=iters, verbose=False, **kwargs
        )
        ar_order = self.order[0]
        self._params = np.hstack(
            (pos_back[:ar_order][::-1], pos_back[ar_order:-1][::-1], pos_back[-1])
        )
        self._endog = endog_reverse
        backcast = self.predict(sum(self.order))
        self._backcast = (
        ...
        # *        ma.L1 => ma.L2 => ma.L3 ...)
        self._params = np.hstack(
            (pos[:ar_order][::-1], pos[ar_order:-1][::-1], pos[-1])
        )
    ...
    def predict(self, step):
        ...
        ma_params = self.params[ar_order:-1][::-1]
        ...
        upper = self._endog["Upper"]["data"][-ar_order:]
        lower = self._endog["Lower"]["data"][-ar_order:]

        pred_upper = np.array([])
        pred_lower = np.array([])
        for i in range(step):
            pred_per_lag_u = (
                upper[-ar_order:].dot(ar_params)
                + max_error
                - resid_upper[-ma_order:].dot(ma_params)
            )
            pred_per_lag_l = (
                lower[-ar_order:].dot(ar_params)
                + min_error
                - resid_lower[-ma_order:].dot(ma_params)
            )

            resid_upper = np.append(resid_upper, max_error)
            resid_lower = np.append(resid_lower, min_error)

            upper = np.append(upper, pred_per_lag_u)
            lower = np.append(lower, pred_per_lag_l)
    ...
    def _fits_resids(self):
        ...
        rec_time = (n_obs - ar_order) if back_upper is None else (n_obs + ma_order)
        for i in range(rec_time):
            fitted_per_lag_u = (
                upper[i : i + ar_order].dot(ar_params)
                + max_error
                - resid_upper[-ma_order:].dot(ma_params)
            )
            fitted_per_lag_l = (
                lower[i : i + ar_order].dot(ar_params)
                + min_error
                - resid_lower[-ma_order:].dot(ma_params)
            )
    ...
    def summary(self):
        ...
        ar_params = self.params[:ar_order]
        ma_params = self.params[ar_order:-1]
        std_param = self.params[-1]

        rows = [[f"ar.L{i+1}", ar_params[i]] for i in range(len(ar_params))]
        ma_rows = [[f"ma.L{i+1}", ma_params[i]] for i in range(len(ma_params))]
        std_row = ["sigma", std_param]
